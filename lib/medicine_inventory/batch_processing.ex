defmodule MedicineInventory.BatchProcessing do
  @moduledoc """
  ...
  alias MedicineInventory.BatchProcessing.Entry
  alias MedicineInventory.Jobs.AnalyzeMedicinePhotoJob
    ...
    if approved_entries == [] do
      ...
      results =
        Enum.map(approved_entries, fn entry ->
          case MedicineInventory.Medicines.create_medicine(entry.ai_results) do
            {:ok, medicine} ->
              # Clean up the photo file after successful save
              if entry.photo_path do
                MedicineInventory.FileManager.delete_file(entry.photo_path)
              end
